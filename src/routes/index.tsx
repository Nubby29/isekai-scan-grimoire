...
import { Aperture, BookOpen, Camera, ChevronLeft, ChevronRight, Coins, Download, RefreshCw, Share2, Shield, Sparkles, Upload, Volume2, VolumeX, X } from "lucide-react";
...
function Codex(props: { items: SavedAppraisal[]; allItems: SavedAppraisal[]; rankFilter: string; setRankFilter: (v: string) => void; elementFilter: string; setElementFilter: (v: string) => void; elements: string[]; dateFilter: string; setDateFilter: (v: string) => void; onOpen: (item: SavedAppraisal) => void }) {
  return <section className="codex"><div className="codex-title"><span>記録</span><div><p>THE APPRAISER'S</p><h2>Grimoire</h2></div><strong>{props.allItems.length} RELICS</strong></div><div className="filters"><select aria-label="Filter by rank" value={props.rankFilter} onChange={(e) => props.setRankFilter(e.target.value)}><option>All</option>{Object.keys(ranks).map((rank) => <option key={rank}>{rank}</option>)}</select><select aria-label="Filter by element" value={props.elementFilter} onChange={(e) => props.setElementFilter(e.target.value)}><option>All</option>{props.elements.map((element) => <option key={element}>{element}</option>)}</select><select aria-label="Filter by date" value={props.dateFilter} onChange={(e) => props.setDateFilter(e.target.value)}><option>All time</option><option>Today</option><option>This week</option></select></div>{props.items.length ? <div className="relic-grid">{props.items.map((item) => <Button variant="ghost" className={`relic-card ${ranks[item.rank].glow}`} key={item.id} aria-label={`Inspect ${item.title}`} onClick={() => props.onOpen(item)}><div className="relic-image-frame">{item.thumbnail ? <img src={item.thumbnail} alt={item.object} /> : <div className="relic-placeholder" aria-hidden="true">鑑</div>}</div><div className="relic-content"><div className="relic-heading"><span className="rarity-badge">{item.rank} <b>[{item.grade}]</b></span><ChevronRight className="inspect-icon" aria-hidden="true" /></div><h3>{item.title}</h3><div className="element-tags">{item.elements.map((element) => <span key={element}>{element}</span>)}</div><div className="relic-meta"><span title="Market value"><Coins aria-hidden="true" />{item.gold}<b>G</b> {item.silver}<b>S</b></span><span title="Durability"><Shield aria-hidden="true" />{item.durability}</span><span title="Mana potency"><Sparkles aria-hidden="true" />{item.mana}</span></div><small>{new Date(item.date).toLocaleDateString()}</small></div></Button>)}</div> : <div className="empty-codex"><BookOpen /><h3>No relics answer your filters</h3><p>Return to the arcane lens and appraise something worthy.</p></div>}</section>;
}
